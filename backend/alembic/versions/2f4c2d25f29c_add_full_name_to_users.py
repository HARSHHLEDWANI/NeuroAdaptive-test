...
def upgrade() -> None:
    ...
    # These drops were autogenerated against a database that
    # Base.metadata.create_all had already populated, so Alembic saw the
    # tables as "extra" and emitted drops for tables no upgrade ever created.
    # On a fresh database the chain died here.
    #
    # Guarded rather than deleted: for any database that already applied this
    # revision the tables are long gone, so skipping is a no-op there, while a
    # fresh database can now get past this step.
    inspector = sa.inspect(op.get_bind())
    if inspector.has_table('paragraphs'):
